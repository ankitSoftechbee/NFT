...
const TeamAffiliate = () => {
    ...
    const {
        ...
        isError,
        error,
    } = metaBullApi.useTeamAffiliateQuery({
    ...
    return (
        <div className="w-full mx-auto">
            ...
            <h2 className="text-lg font-semibold text-white text-left mt-10">My Team</h2>
                        ...
                        <div key={index} className="bg-[#1d1d1f] rounded-2xl border border-emerald-500/20 p-4 shadow-lg ">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end space-y-2 sm:space-y-0">
                                <div className="space-y-2 w-full">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-sm text-gray-300 border-t border-emerald-500/20 pt-3">
                                        <div>
                                            <span className="text-gray-500">Name:</span> {member.name}
                                        </div>
                                        <div>
                                            <span className="text-gray-500">Referrer:</span> {member.reffname}
                                        </div>
                                        <div>
                                            <span className="text-gray-500">Referrer ID:</span> {member.reffid}
                                        </div>
                                        <div>
                                            <span className="text-gray-500">Join Amount:</span>
                                            {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(member.joinAmount)}
                                        </div>
                                    </div>

                                    <div className="flex justify-between items-center mt-2">
                                        <div className="text-sm text-gray-300">
                                            <span className="text-gray-500">Joined:</span> {formatDate(member.dateofjoining)}
                                        </div>
                                    </div>
                                </div>
                            </div>
